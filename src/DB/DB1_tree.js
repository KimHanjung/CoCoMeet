...
const newApple = (room_id, tree_id, text, parent) => {
...
const deleteBlock = (room_id, node_id) => {
...
const editColor = (room_id, node_id, color) => {
...
const editDeco = (room_id, node_id, deco) => {
    const target = Total_TREES.filter(node => node.room_id === room_id && node.node_id === node_id);
    let newNode = target[0];
    newNode["deco"] = deco;
...
const editWeight = (room_id, node_id, weight) => {
    const target = Total_TREES.filter(node => node.room_id === room_id && node.node_id === node_id);
    let newNode = target[0];
